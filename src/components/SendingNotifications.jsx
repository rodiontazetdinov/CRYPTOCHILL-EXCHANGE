...
export const SendingNotifications = () => {
  ...
  return (
    <form className=" flex flex-col bg-order pt-12 px-8 rounded-3xl" onSubmit={handleSubmit}>
        <h3 className="text-5xl text-center font-bold font-segoe bg-gradient-to-r from-[#FBE3F1] to-[#CAAEFF] bg-clip-text text-transparent mb-4 self-center">Уведомления о статусе заказа</h3>
        <p className="text-xl font-semibold mb-6">Введите свой email, если хотите получать уведомления о изменении статуса этого заказа</p>
        ...
        <button type="submit" className={classNames("font-semibold text-xl px-4 py-3 bg-btns rounded-lg self-left mt-4 w-full ")}>Включить уведомления</button>
        <img className="self-center" src={bankingImg} alt="картинка банкинга"/>
    </form>
  );
};
